...
- **Client Portal** (Pro+) — self-service login for landlords and management companies to view their portfolio (offices, leases with expiration/notice dates, and maintenance work orders — read-only, financials hidden), manage their secondary contacts, upload/download/remove their own documents, and submit profile change requests for staff approval; admins can revoke or rotate access and review pending requests
...
- **AI Automation (Google Gemini)** — configurable Gemini model (`GEMINI_MODEL`, `GEMINI_API_KEY`, `GEMINI_API_BASE`) powers lease-document ingestion (extract key terms to pre-fill the lease form, or — for a prior/expired lease, amendment or reconciliation statement — extract the year-by-year rent and CAM schedule into the lease's historical CAM schedule instead of overwriting the active lease's numbers; available on all tiers. Documents up to `AI_MAX_FILE_SIZE_MB` (75 MB, matching the `MAX_FILE_SIZE_MB` ceiling for stored attachments) are handled by extracting their text locally, splitting scanned PDFs into page ranges, and merging the per-segment results — including merging a rent schedule that spans several segments), AI lease-abstract suggestions, and narrative weekly/monthly briefings of upcoming notice periods, expirations, and maintenance. Richer AI beyond basic ingestion is gated to **Pro** and above (`ai_assist`); the service degrades gracefully when no API key is configured
...
- **Historical CAM schedule import** — onboarding an existing tenancy with years of prior leases, amendments and reconciliation statements: AI document extraction (`POST /ai/leases/parse-history`) or a spreadsheet upload proposes per-year rows (base rent, CAM, operating expenses, escalation, reconciliation true-up), which are reviewed and then explicitly imported onto the lease's CAM schedule as provenance-tagged **historical** rows. Importing history never touches the active lease's own financial terms — the active lease stays the source of truth for the current period, and copying a row's figures onto it is a separate, audited **promote** action. Each import shares an `import_batch_id` so a bad import can be reverted wholesale — see [docs/cam-history-import.md](docs/cam-history-import.md)
