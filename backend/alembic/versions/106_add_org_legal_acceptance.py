"""Add legal acceptance columns to organizations

Captures the org admin's acceptance of the required legal documents (Terms of
Service, EULA, Privacy Policy, Acceptable Use Policy) at signup:
``legal_accepted_at`` records when acceptance happened and
``legal_accepted_versions`` snapshots the ``slug -> version`` map that was
accepted, so we can prove which version of each document an organization agreed
to.
